...
@Slf4j
@Component
public class FinanceWebLoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        ...
        if (FinanceUtils.isAjaxRequest(request)) {
            FinanceResponse data = new FinanceResponse();
            if (savedRequest == null) {
                FinanceUtils.makeFailureResponse(response, data.message("请通过授权码模式跳转到该页面"));
                return;
            }
            data.data(savedRequest.getRedirectUrl());
            FinanceUtils.makeSuccessResponse(response, data);
        } else {
            if (savedRequest == null) {
