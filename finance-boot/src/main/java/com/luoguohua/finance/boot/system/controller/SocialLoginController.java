...
public class SocialLoginController {
    ...
    public void renderAuth(@PathVariable String oauthType, @PathVariable String type, HttpServletResponse response) throws IOException, FinanceException {
        AuthRequest authRequest = socialLoginService.renderAuth(oauthType);
        response.sendRedirect(authRequest.authorize(oauthType + FinanceUtils.getDoubleColon() + AuthStateUtils.createState()) + FinanceUtils.getDoubleColon() + type);
    }
    ...
    public String login(@PathVariable String oauthType, AuthCallback callback, String state, Model model) {
        try {
            FinanceResponse febsResponse = null;
            String type = StringUtils.substringAfterLast(state, FinanceUtils.getDoubleColon());
            ...
            String errorMessage = FinanceUtils.containChinese(e.getMessage()) ? e.getMessage() : "第三方登录失败";
    ...
    public FinanceResponse bindLogin(@Valid BindUser bindUser, AuthUser authUser) throws FinanceException {
        OAuth2AccessToken oAuth2AccessToken = this.socialLoginService.bindLogin(bindUser, authUser);
        return new FinanceResponse().data(oAuth2AccessToken);
    }
    ...
    public FinanceResponse signLogin(@Valid BindUser registUser, AuthUser authUser) throws FinanceException {
    ...
    public FinanceResponse findUserConnections(@NotBlank(message = "{required}") @PathVariable String username) {
        List<UserConnection> userConnections = this.socialLoginService.findUserConnections(username);
        return new FinanceResponse().data(userConnections);
    }
}
